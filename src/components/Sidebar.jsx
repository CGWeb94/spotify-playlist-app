  ...
  return (
    <aside className="app-sidebar">
      ...
      <nav className="sidebar-nav">
        <button className={`nav-item ${selected === "home" ? "active" : ""}`} onClick={() => onSelect("home")}>
          <i className="fa-solid fa-house me-2"></i> Home
        </button>

        <button className={`nav-item ${selected === "playlists" ? "active" : ""}`} onClick={() => onSelect("playlists")}>
          <i className="fa-solid fa-list me-2"></i> Deine Playlists <span className="badge bg-secondary ms-auto">{counts.playlists ?? 0}</span>
        </button>
        ...
        <button className={`nav-item ${selected === "released" ? "active" : ""}`} onClick={() => onSelect("released")}>
          <i className="fa-solid fa-calendar me-2"></i> Nach Jahr erschienen <span className="badge bg-secondary ms-auto">{counts.releasedYears ?? 0}</span>
        </button>
      </nav>
    </aside>
  );
